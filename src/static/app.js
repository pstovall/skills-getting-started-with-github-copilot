document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchActivities() {
    try {
      ...
      // Reset activity select to default option to avoid duplicated entries
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsList = details.participants.length > 0
          ? `<ul class="participants-list">${details.participants.map(p => `<li>${p} <span class='delete-icon' title='Remove' data-activity='${name}' data-participant='${p}'>&#128465;</span></li>`).join('')}</ul>`
